...
const uint8_t CDC_Payload_WaitBoot[3] = {0x11, 0x60, 0x06};
...
const uint8_t CDC_Payload_OperateStandby[6] = {0x20, 0x01, 0x03, 0x09, 0x05, 0x01};
...
uint8_t CDC_Payload_OperatePlaying[11] = {0x47, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00};
...
void TLCDCEmu::init(char *btName)
{
	...
	btSink.set_stream_reader(read_data_stream, false);
	btSink.set_auto_reconnect(true);
	btSink.start("Tuner List CDC Emulator");
...
void TLCDCEmu::readHU(const uint8_t *data, uint16_t length)
{
	for (int i = 0; i < length; i++)
	{
		...
		{ // if not a new incoming packet header drop transmission
			CDC_RX_Ptr = 0;
		}
		...
		{ // handle incoming transmission
			...
			{ // if received data to long, drop transmission
				CDC_RX_Ptr = 0;
			}
		}
